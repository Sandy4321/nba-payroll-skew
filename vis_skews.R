...
ggplot(perf_skew, aes(x=skew, y=reg_season_wp)) + 
    geom_point() + 
    geom_smooth(method=lm, se=FALSE)
ggsave(file="reg_season_by_skew.png", width=10, height=6)
...
ggplot(perf_skew, aes(x=skew, y=playoff_wp)) + 
    geom_point() + 
    geom_smooth(method=lm, se=FALSE)
ggsave(file="playoff_by_skew.png", width=10, height=6)
